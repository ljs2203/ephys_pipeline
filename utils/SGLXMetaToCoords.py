...
def CoordsToKSjson(meta, chans, xCoord, yCoord, connected, shankInd, shankSep, baseName, savePath, buildPath ): 
    ...
    # channel map is the order of channels in the file, rather than the 
    # original indicies of the channels
    chanMap = np.arange(0,nChan,dtype='int').tolist()
    
    xCoordList = (shankInd*shankSep + xCoord).tolist()
    yCoordList = yCoord.tolist()

    kcoords = (shankInd).astype('float64').tolist()  # convert to 1-based for MATLAB
